      ...
      <main className="max-w-4xl mx-auto px-4 pt-8">
        ...
        {data.length > 0 && (
          <section className="mb-10">
            <h2 className="text-lg font-semibold text-gray-200 mb-4">Portfolio Allocation</h2>
            <DonutChart data={data} totalValue={totalValue} />
          </section>
        )}
              ...
              <Card key={item.rowIndex} className="group hover:border-gray-700 transition-all relative overflow-hidden bg-gray-900/50 hover:bg-gray-900">
                <div className="p-5">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    {/* Middle: Stats */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6 text-sm flex-1">
                      <div>
                        <span className="block text-gray-500 text-[10px] uppercase font-bold tracking-wider mb-1">Current Price</span>
                        <span className="font-medium text-gray-200">{formatCurrency(item.price)}</span>
                      </div>
                      <div>
                        <span className="block text-gray-500 text-[10px] uppercase font-bold tracking-wider mb-1">Acquired Price</span>
                        <span className="font-medium text-gray-200">{formatCurrency(item.costBasis)}</span>
                      </div>
                      <div>
                  ...
                  {/* Sparkline line chart (inside the card) */}
                  <div className="hidden group-hover:block animate-in fade-in slide-in-from-top-2 duration-300">
                    <AssetLineChart currentPrice={item.price} ticker={item.ticker} />
                  </div>
